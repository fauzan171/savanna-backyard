...
// Parse notes string "[timestamp] content\n[timestamp] content" into array
function parseNotesString(notes: string) {
  // Try JSON parse first (in case format changes later)
  try {
    const parsed = JSON.parse(notes);
    if (Array.isArray(parsed)) return parsed;
  } catch {
    // Not JSON, parse as plain string format
  }

  // Parse "[2026-03-04T07:00:00.000Z] content" format
...
// Normalize notes dari berbagai format ke bentuk array seragam
function normalizeNotes(notes: LeadNote[] | string | null) {
  if (!notes) return [];
...
export function LeadDetail({ lead, onEdit, onConvert }: LeadDetailProps) {
  ...
  // Normalize notes ke timeline items
  const noteItems = normalizeNotes(lead.notes).map((note) => ({
    id: note.id,
    title: note.createdBy?.name ?? "Staff",
