...
export class MaintenanceRepository {
	...
	async findById(id: string): Promise<typeof maintenanceRecords.$inferSelect | null> {
	...
	async create(data: Omit<typeof maintenanceRecords.$inferInsert, 'id'>): Promise<typeof maintenanceRecords.$inferSelect> {
	...
	async update(
		id: string,
		data: Partial<Omit<typeof maintenanceRecords.$inferInsert, 'id' | 'createdAt' | 'createdBy'>>
	...
	async findByVehicleId(vehicleId: string, limit = 50): Promise<typeof maintenanceRecords.$inferSelect[]> {
		return this.db
			.select()
			.from(maintenanceRecords)
	...
	async findActiveByVehicleId(vehicleId: string): Promise<typeof maintenanceRecords.$inferSelect | null> {
		const result = await this.db
			.select()
			.from(maintenanceRecords)
			.where(
	...
	async findUpcoming(days: number): Promise<typeof maintenanceRecords.$inferSelect[]> {
