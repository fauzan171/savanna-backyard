...
export const paymentsServicesMiddleware = () => async (c: Context<PaymentsEnv>, next: () => Promise<void>) => {
	...
	const paymentsService = new PaymentsService(paymentsRepository, bookingsRepository);
...
const handleWebhookHandler = async (c: Context<PaymentsEnv>) => {
	...
	try {
		...
		// TODO: Update payment status in database based on webhook result
...
export function createPaymentsRouter(): Hono<PaymentsEnv> {
	...
	// Verify payment
	router.post('/:id/verify', validateBody(verifyPaymentSchema), verifyPaymentHandler);

	// Reject payment
	router.post('/:id/reject', validateBody(rejectPaymentSchema), rejectPaymentHandler);
